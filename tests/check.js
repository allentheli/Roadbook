...
// brand and rename checks
{
  ...
  // (a) the old name survives only in the two "formerly" lines and the redirect stub
  for (const f of fs.readdirSync(root).filter(n => n.endsWith('.html'))){
    fs.readFileSync(path.join(root, f), 'utf8').split('\n').forEach((line, i) => {
      if (/ONCourse/.test(line) && !/Roadbook was previously called ONCourse\./.test(line)) errors.push(`${f}:${i + 1} old name in user-visible copy`);
    });
  }
