...
- `tests/check.js` — dependency-free library check. Run `node tests/check.js` before every commit; do not commit if it reports errors. GitHub runs the same check on every push and pull request (`.github/workflows/check.yml`), so a pull request shows a red cross if it was skipped. It also verifies that every page loads `regimens.js?v=<APP_VERSION>`, so when you bump `APP_VERSION`, update the `?v=` stamp in all HTML pages (this busts the GitHub Pages cache for the data).
- `hero.png`, `hero-ev303.png`, `hero-flot.png`, `compare.png`, `decision.png`, `route-kn522.png` — the landing-page examples, rendered from the builder; each has a lossless `.webp` twin that the page lists first (`python3 tools/webp-landing.py` regenerates the twins after a re-render; `tests/check.js` warns when a PNG is newer than its twin).
...
1. Retrieve a source for every schedule-bearing claim: the agents named, the cycle length,
   the number of cycles or duration, and the start timing of each phase. Do not encode any
   of these from existing knowledge. Verify against the primary publication and a structured
   protocol reference (HemOnc.org, eviQ) and current FDA labeling. Cite both in `refs` as
   `{t: citation text, q: PubMed search terms}`.
...
9. Append to `verification-log.md`: for each pathway, which source you opened and how far you
   got (full text, abstract only, protocol record, or label), what you checked against it,
   and anything you could not confirm. If you could not retrieve a source for a claim, say so
   explicitly and name what blocked you. Do not silently fall back on existing knowledge.
